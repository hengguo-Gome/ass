...
public final class JsonUtil {
	...
	public static Map<String, String> convertJsonToMap(String jsonString,String elementName) {
		...
		Document doc = null;
		try {
			inputStream = new ClassPathResource("/redis/redisData.xml")
					.getInputStream();
		...
		Element resultMap = rootElement.element(elementName);
		List<Element> elements = resultMap.elements();
		for (Element element :elements) {
			String jsonKey = element.attribute("jsonKey").getText();
			String mapKey = element.attribute("mapKey").getText();
			String jsonValue = (String)jsonObject.get(jsonKey);
			if(StringUtils.isNotBlank(jsonValue)){
				returnMap.put(mapKey, jsonValue);
			}
		}
		return returnMap;
