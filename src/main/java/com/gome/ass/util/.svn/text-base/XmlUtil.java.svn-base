...
public class XmlUtil {
	...
	private static String resouceBasePath = "template/";
	...
	public String genXmlByTemplate(String templateNamePath,Map<String,? extends Object> context) throws ResourceNotFoundException, ParseErrorException, Exception{
		Template template = veEngine.getTemplate(resouceBasePath+templateNamePath);
	...
	public List<Map<String, List<Map<String, String>>>> divisionXmlToMap(String templateName,String separator,Document doc) {
		List<Map<String, List<Map<String, String>>>> returnData = new ArrayList<Map<String, List<Map<String, String>>>>();
		List<Document>  divisionedDocs = this.divisionDomByXPath(separator, doc);
		for(Document divisionedDoc:divisionedDocs){
			List<Map<String, String>> parseData = this.parseXML(divisionedDoc);
			Map<String, String> templateMap = this.loadAndParseTemplate(templateName);
			List<Map<String, List<Map<String, String>>>> conversionMap = this.conversionProcess(parseData, templateMap);
			returnData.add(conversionMap.get(0));
		}
		return returnData;
	}
	...
	private InputStream loadTemplate(String templateName) {
		return this.getClass().getClassLoader().getResourceAsStream(XmlUtil.resouceBasePath+templateName);
	}
	...
	private List<Document> divisionDomByXPath(String xPath,Document srcDom){
	 ...
	 * @return
	 */
	public static List<String> parseByXpath(Document doc,String xpath){
	    List<String> returnLists = new ArrayList<String>();
        try{
            Element root = doc.getRootElement();
            XPath xpathObj = XPath.newInstance(xpath);
            List<Element> list = xpathObj.selectNodes(root);
            for(Element e:list){
                returnLists.add(e.getText());
            }
        }catch (Exception e) {
             e.printStackTrace();
        }
	...
	public static void main(String[] args) throws Exception{
        ...
        Document read = null;
        try {
            inputStream = new ClassPathResource("jlMessage.xml").getInputStream();
            
            SAXReader sax = new SAXReader();
            read = XmlUtil.getInstance().getDocument(inputStream);
            System.out.println(Arrays.asList(parseByXpath(read, "//GRDM")));
        } catch (Exception e) {
            e.printStackTrace();
        }
