...
import com.gome.ass.service.wsdl.service.AppSyncServiceStub;
import com.gome.ass.service.wsdl.service.AppSyncServiceStub.ArrayOfString;
import com.gome.ass.service.wsdl.service.AppSyncServiceStub.GetBatchNumbersByAppCodeAndBatchNumber;
import com.gome.ass.service.wsdl.service.AppSyncServiceStub.GetUsersByAppCodeAndBatchNumber;
...
public class GetAccountJob {
	...
	private static String SYS_CODE = "ass";
	...
	private Logger log = LoggerFactory.getLogger(GetAccountJob.class);
	...
	public void getAccount() {
		...
		try {
			...
			if(batchNos!=null&&batchNos.length==1){
				...
				dr.setXmlContent(resultStr);
				parserXml(resultStr);
				saveBatchNo();
				dr.setId(UUIDUtil.getUUID());
		        dr.setDateSend(DateUtils.toDate(batchNo, DateUtils.SYSVAR_PATTERN));
